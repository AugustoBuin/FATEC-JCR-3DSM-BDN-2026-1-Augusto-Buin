...
const existingLoja = LojaEntity.restore({ name: 'Loja', city: 'Cidade' }, LOJA_ID);
...
describe('CreateTimeUseCase', () => {
  it('creates and persists a time when loja exists', async () => {
    ...
    const useCase = new CreateTimeUseCase(timeRepo, lojaRepo);
    const time = await useCase.execute({ name: 'Time Alpha', lojaId: LOJA_ID });
  ...
  it('throws DomainError when loja does not exist', async () => {
    ...
    const useCase = new CreateTimeUseCase(timeRepo, lojaRepo);
    await expect(
      useCase.execute({ name: 'Time Alpha', lojaId: LOJA_ID }),
